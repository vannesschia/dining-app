    ...
    for offering in ai_output:
        ...
        for item in offering["items"]:
            if item in raw_scraper_items.keys():
                # Aggregate traits and allergens
                traits.update(raw_scraper_items[item]["traits"])
                ...
                to_add_trans_fat_g = raw_scraper_items[item]['nutrition'].get('Trans Fat', {}).get('value', 0)
                total_fat_g = (total_fat_g or 0) + to_add_trans_fat_g if to_add_trans_fat_g is not None else 0
    ...
    for raw_item_name, raw_item in raw_scraper_items.items():
        if raw_item_name not in covered_item_names:
            ...
            solver_variables.append({
                ...
                "traits": raw_item['traits'],
                "allergens": raw_item['allergens'],
                "dining_hall_id": dining_hall_id,
                "meal_period": meal_period.lower(),
                "station": station_name,
                "portion_size": raw_item.get('nutrition', {}).get('Serving Size', {}).get('portion_size', None),
                "serving_size_g": int(raw_item.get('nutrition', {}).get('Serving Size', {}).get('value', None)),
                "calories_kcal": int(raw_item.get('nutrition', {}).get('Calories', {}).get('value', None)),
                "total_fat_g": raw_item.get('nutrition', {}).get('Total Fat', {}).get('value', None),
                "saturated_fat_g": raw_item.get('nutrition', {}).get('Saturated Fat', {}).get('value', None),
                "trans_fat_g": raw_item.get('nutrition', {}).get('Trans Fat', {}).get('value', None),
                "cholesterol_mg": raw_item.get('nutrition', {}).get('Cholesterol', {}).get('value', None),
                "sodium_mg": raw_item.get('nutrition', {}).get('Sodium', {}).get('value', None),
                "total_carbohydrate_g": raw_item.get('nutrition', {}).get('Total Carbohydrate', {}).get('value', None),
                "dietary_fiber_g": raw_item.get('nutrition', {}).get('Dietary Fiber', {}).get('value', None),
                "sugars_g": raw_item.get('nutrition', {}).get('Sugars', {}).get('value', None),
                "protein_g": raw_item.get('nutrition', {}).get('Protein', {}).get('value', None),
                "convenience_score": 1,
                "type": "entree"
            })
